  ...
  return (
    <Widget>
      ...
      <Widget.Content>
        ...
        <ul>
          {results.map((result, index) => <DetailedResult result={result} index={index} key={index}/>)}
        </ul>
      </Widget.Content>
    </Widget>
  )
...
function simulateWaiting (callback) {
  setTimeout(() => {
    callback()
  }, 1 * 1000)
}
...
export default function QuizPage () {
  ...
  React.useEffect(() => {
    simulateWaiting(() => setScreenState(screenStates.QUIZ))
  }, [])
